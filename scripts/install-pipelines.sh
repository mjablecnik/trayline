...
sed 's/\r$//' "$REPO_DIR/scripts/trayline-agent" > "$TRAYLINE_HOME/trayline-agent"
chmod +x "$TRAYLINE_HOME/trayline-agent"
echo "    Installed trayline-agent"

sed 's/\r$//' "$REPO_DIR/scripts/sync.sh" > "$TRAYLINE_HOME/sync.sh"
chmod +x "$TRAYLINE_HOME/sync.sh"
echo "    Installed sync.sh"

sed 's/\r$//' "$REPO_DIR/.rsyncignore" > "$TRAYLINE_HOME/.rsyncignore"
