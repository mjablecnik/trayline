...
- [ ] 1. Add `variables` field to rawPipeline and split ParsePipeline into parse + validate phases
  - [ ] 1.1 Add `Variables map[string]string` field with `yaml:"variables"` tag to `rawPipeline` struct in `pipeline.go`
    - _Requirements: 1.1, 1.2_
  - [ ] 1.2 Extract a new exported `ParsePipelineRaw(path string) (*Pipeline, map[string]string, error)` function that parses YAML and returns the pipeline and variable map without running validation
    - Return empty map (not nil) when `variables` section is absent
    - _Requirements: 1.3_
  - [ ] 1.3 Export validation as `ValidatePipeline(p *Pipeline) error` (rename existing `validatePipeline`)
    - Update all internal callers to use the new name
    - _Requirements: 3.3_
  - [ ] 1.4 Update `ParsePipeline` to call `ParsePipelineRaw` then `ValidatePipeline` to preserve backward compatibility
    - _Requirements: 1.1, 1.2, 1.3_

- [ ] 2. Create `variables.go` with core variable functions
  - [ ] 2.1 Implement `ParseCLIVars(flags []string) (map[string]string, error)`
    - Split each flag on the first `=`; error if no `=` found
    - Last occurrence of a key wins
    - _Requirements: 2.1, 2.4, 2.5_
  - [ ] 2.2 Implement `MergeVariables(yamlVars, cliVars map[string]string) map[string]string`
    - CLI values take precedence over YAML values
    - Keys from both maps are included in the result
    - _Requirements: 2.2, 2.3_
  - [ ] 2.3 Implement `FindPlaceholders(s string) []string`
    - Use regex `\{\{([a-z0-9-]+)\}\}` to extract placeholder keys
    - Return unique keys found in the string
    - _Requirements: 3.1_
  - [ ] 2.4 Implement `ResolveString(s string, vars map[string]string) string`
    - Replace all `{{key}}` placeholders with corresponding values from vars
    - Preserve surrounding text
    - _Requirements: 3.4, 3.5_
  - [ ] 2.5 Implement `SubstituteVariables(p *Pipeline, vars map[string]string) error`
  ...
  - [ ]* 2.6 Write property test for CLI variable parsing (Property 2: CLI variable parsing with last-wins semantics)
    - **Property 2: CLI variable parsing with last-wins semantics**
    - **Validates: Requirements 2.1, 2.5**
  - [ ]* 2.7 Write property test for malformed --var flag rejection (Property 4: Malformed --var flag rejection)
    - **Property 4: Malformed --var flag rejection**
    - **Validates: Requirements 2.4**
  - [ ]* 2.8 Write property test for variable merge precedence (Property 3: Variable merge — CLI takes precedence)
    - **Property 3: Variable merge — CLI takes precedence**
    - **Validates: Requirements 2.2, 2.3**
  - [ ]* 2.9 Write property test for template substitution correctness (Property 5: Template substitution correctness)
    - **Property 5: Template substitution correctness**
    - **Validates: Requirements 3.1, 3.2, 3.4, 3.5, 3.6, 6.1, 6.2**
  - [ ]* 2.10 Write property test for undefined variable detection (Property 6: Undefined variable detection reports all undefined keys)
...
- [ ] 4. Integrate `--var` flag into `main.go` and wire the resolution flow
  - [ ] 4.1 Add a custom `varFlags` type implementing `flag.Value` and register `--var` flag in `run()`
    - Update `usageText()` to document the `--var` flag
    - _Requirements: 2.1_
  - [ ] 4.2 Replace `ParsePipeline` call with `ParsePipelineRaw` + `ParseCLIVars` + `MergeVariables` + `SubstituteVariables` + `ValidatePipeline` sequence
  ...
  - [ ] 4.3 Add `ResolvedVars map[string]string` field to `Executor` struct in `executor.go`
    - _Requirements: 5.1_

- [ ] 5. Update dry-run output to display resolved variables
  - [ ] 5.1 Modify `printDryRun()` in `executor.go` to print a "Variables" section before steps, listing each resolved variable key and value
  ...
  - [ ]* 5.2 Write property test for dry-run resolved values display (Property 7: Dry-run shows resolved values, not placeholders)
    - **Property 7: Dry-run shows resolved values, not placeholders**
    - **Validates: Requirements 5.1, 5.2**

- [ ] 6. Add YAML round-trip property test and unit tests
  - [ ]* 6.1 Write property test for variables YAML round trip (Property 1: Variables YAML round trip)
    - **Property 1: Variables YAML round trip**
    - **Validates: Requirements 1.1, 1.2**
  - [ ]* 6.2 Write unit tests for variable edge cases in `variables_test.go`
