...
for dir in tasks processes workflows; do
  ...
  for f in "$REPO_ROOT"/pipelines/$dir/*.yaml(N); do
...
# Copy lifecycle.yaml
if [[ -f "$REPO_ROOT/pipelines/lifecycle.yaml" ]]; then
  dest="$TRAYLINE_HOME/pipelines/lifecycle.yaml"
  if [[ ! -f "$dest" ]] || [[ "$REPO_ROOT/pipelines/lifecycle.yaml" -nt "$dest" ]]; then
    cp "$REPO_ROOT/pipelines/lifecycle.yaml" "$dest"
