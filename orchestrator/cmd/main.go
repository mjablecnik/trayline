...
func runWithLifecycle(executor *engine.Executor, lifecyclePath string, pipelineName string, vars map[string]string, verbose bool) int {
	...
	for i := 0; i < len(lc.Before); i++ {
		...
		var exitCode int
		if step.Agent != "" {
			_, exitCode, err = runner.RunAgent(step.Agent, step.Prompt, step.Model, step.Effort, projectDir, env, stepVerbose, os.Stdout, os.Stderr)
	...
	for i := 0; i < len(lc.After); i++ {
		...
		var stepErr error
		if step.Agent != "" {
			_, stepExitCode, stepErr = runner.RunAgent(step.Agent, prompt, step.Model, step.Effort, projectDir, env, stepVerbose, os.Stdout, os.Stderr)
		} else if command != "" {
