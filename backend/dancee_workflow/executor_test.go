...
func TestLoopIterationControl(t *testing.T) {
	rapid.Check(t, func(rt *rapid.T) {
		...
		// Generate one decision per potential iteration
		decisions := make([]bool, maxIter)
		for i := range decisions {
			decisions[i] = rapid.Bool().Draw(rt, fmt.Sprintf("decision-%d", i))
		}

		// Calculate expected iterations:
		// Loop continues while decision is true; stops on first false or max_iterations.
		expectedIter := maxIter // default: all iterations (all decisions are true)
		...
		// Provide enough runner responses (one per loop step per iteration)
		...
		pipeline := &Pipeline{
			Elements: []PipelineElement{
				{Loop: &Loop{
					...
					Steps: []Step{
						{Name: "loop-step", Command: "echo test"},
					},
					Condition: Condition{Prompt: "Continue?"},
				}},
			},
		...
		if len(runner.calls) != expectedIter {
			rt.Fatalf("expected %d iterations, got %d (maxIter=%d, decisions=%v)",
				expectedIter, len(runner.calls), maxIter, decisions)
		}
	})
}
...
func TestStepConditionRouting(t *testing.T) {
	rapid.Check(t, func(rt *rapid.T) {
		...
		decision := rapid.Bool().Draw(rt, "decision")

		cond := &Condition{Prompt: "Should continue?"}
		if hasGoto {
			cond.Goto = "step-c"
		}

		// Pipeline: step-a (with condition) → step-b → step-c
		pipeline := &Pipeline{
			Elements: []PipelineElement{
				{Step: &Step{Name: "step-a", Command: "echo a", Condition: cond}},
				{Step: &Step{Name: "step-b", Command: "echo b"}},
				{Step: &Step{Name: "step-c", Command: "echo c"}},
			},
		}

		runner := &mockRunner{responses: []runResponse{
			{exitCode: 0}, {exitCode: 0}, {exitCode: 0},
		}}
		eval := &mockEvaluator{decisions: []bool{decision}}
		e := buildExecutor(pipeline, runner, eval)
		code := e.Run()
		...
		switch {
		case hasGoto && decision:
			// goto + true: jump to step-c, skipping step-b
			if len(runner.calls) != 2 {
				rt.Fatalf("goto+true: expected 2 calls (step-a + step-c), got %d", len(runner.calls))
			}
			if runner.calls[1].command != "echo c" {
				rt.Fatalf("goto+true: expected step-c to run second, got %+v", runner.calls[1])
			}
		case hasGoto && !decision:
			// goto + false: continue sequentially (step-b, then step-c)
			if len(runner.calls) != 3 {
				rt.Fatalf("goto+false: expected 3 calls, got %d", len(runner.calls))
			}
		case !hasGoto && decision:
			// no goto + true: continue sequentially (step-b, then step-c)
			if len(runner.calls) != 3 {
				rt.Fatalf("no-goto+true: expected 3 calls, got %d", len(runner.calls))
			}
		default:
			// no goto + false: stop after step-a
			if len(runner.calls) != 1 {
				rt.Fatalf("no-goto+false: expected 1 call (stopped by condition), got %d", len(runner.calls))
			}
		}
	})
}
